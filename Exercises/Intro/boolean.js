// Instructions
// In this exercise, you'll be implementing the quest logic for a new RPG game a friend is developing. The game's
// main character is Annalyn, a brave girl with a fierce and loyal pet dog.Unfortunately, disaster strikes, as her best
// friend was kidnapped while searching for berries in the forest.Annalyn will try to find and free her best friend,
...
//      If Annalyn does not have her dog then she and the prisoner must be very sneaky! Annalyn can free the prisoner
//          if the prisoner is awake and the knight and archer are both sleeping, but if the prisoner is sleeping they can't be rescued:
//          the prisoner would be startled by Annalyn's sudden appearance and wake up the knight and archer.
// You have four tasks: to implement the logic for determining if the above actions are available based on the state of the three characters
// found in the forest and whether Annalyn's pet dog is present or not.
...
console.log(canExecuteFastAttack1(true));
...
// Implement a function named canSpy that takes three boolean values, indicating if the knight, archer and the
...
// Implement a function named canSignalPrisoner that takes two boolean values, indicating if the archer and
...
// Implement a function named canFreePrisoner that takes four boolean values. The first three parameters
// indicate if the knight, archer and the prisoner, respectively, are awake. The last parameter indicates if Annalyn's
