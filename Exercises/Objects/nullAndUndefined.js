// Instructions
// It is your job to provide and maintain parts of the IT system of an 
// amusement park. In particular, you are responsible for the program that manages visitors and tickets.
...
// 2. Revoke the ticket
// When people leave the amusement park, their ticket gets revoked. If they come back, 
// they need to buy a new one. To save regular visitors some time, 
// they only need to register once and the visitor information will be kept for subsequent visits.
// That means when a visitor leaves the park, only their ticket should be invalidated but the rest 
...
// To prevent forgery, the ticket identifiers are unique. Once a ticket is printed, its identifier 
// is added as a key in an object in the system so it can be tracked.
// Before the ticket is sold to a visitor, it is stored with the value null in the ticket tracking object. 
// When it is sold to a visitor, the visitor's name is assigned as a value. When employees have doubts about the validity of a ticket, 
...
let ticketStatus = (ticket , ticketId) => {
    if (ticket[ticketId] === null) {
        ...
        console.log("Sold to " + ticket[ticketId]);
      } else {
        console.log("Unknown ticket id");
      }
};

ticketStatus({'0H2AZ123': null, '23LA9T41': 'Verena Nardi'} , "23LA941");
...
let simpleTicketStatus = (ticket , ticketId) => {
    if ( ticket[ticketId]) {
        ...
        console.log("Invalide ticket !!!");
    }
};

simpleTicketStatus({'0H2AZ123': null, '23LA9T41': 'Verena Nardi'}, '0H2AZ123');

// 5. Determine the version of terms and conditions
// Due to new legal requirements, newly created visitor objects now also contain detailed information on the "General Terms & Conditions" 
// (GTC) that the user agreed to. You can see an example of the new visitor object below.
